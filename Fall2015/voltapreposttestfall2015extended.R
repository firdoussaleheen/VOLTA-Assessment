...
#set directory
setwd("D:/RESEARCH/TempleVLA/Data Analysis/Fall2015") 

#import data into workspace
volta <- read.csv("Students_Score_Fall2015_v1.csv", header=TRUE)
...
write.csv(voltasplitControl, file = "voltacontrol_fall2015_v1.csv")
write.csv(voltasplitExperimental, file = "voltaexperimental_fall2015_v1.csv")
#write.table(voltasplit$Control, file = "voltacontrol_fall2015_v1.txt", sep = "\t")


library(ggplot2)

voltacontrol <- read.csv("voltacontrol_fall2015_v1.csv", header=TRUE)
voltaexperimental <- read.csv("voltaexperimental_fall2015_v1.csv", header=TRUE)
...
#png(file="controlfall2015.png",width=800,height=350,antialias="none")
...
#png(file="posttestcontrolfall2015.png",width=400,height=350,antialias="none")
...
#png(file="posttestexperimentalfall2015.png",width=400,height=350,antialias="none")
...
library(Cairo)
tiff(filename="testscorefall2015.tiff", width = 10, height = 7, units = 'in', res = 300, 
compression = "lzw", type = "cairo", antialias = "default")

#CairoTIFF(filename = "testscorefall2015.tiff", width = 3000, height = 2100, units = "px",
#         pointsize = 12, bg = "white", res = 300)

#ggsave(filename = "testscorefall2015.tiff", width = 10, height = 7, units = "in", dpi = 300)
...
#anova
volta$LabID <- factor(volta$LabID)
volta$Student.ID <- factor(volta$StudentID)
volta$DayID <- factor(volta$DayID)
volta$GroupID <- factor(volta$GroupID, labels=c("Experimental","Control"))
voltanova <- aov(Gain ~ GroupID + Error(LabID), data=volta)
summary(voltanova)
